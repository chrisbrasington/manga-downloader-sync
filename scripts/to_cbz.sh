...
while IFS= read -r f; do
  [[ "$f" == *.rar ]] && need_unrar=true && break
done < <(find "$DIR" -type f \( -iname "*.zip" -o -iname "*.rar" \) | sort)

if $need_unrar && ! command -v unrar &>/dev/null && ! command -v 7z &>/dev/null; then
...
mapfile -t FILES < <(find "$DIR" -type f \( -iname "*.zip" -o -iname "*.rar" \) | sort)

if [[ ${#FILES[@]} -eq 0 ]]; then
  echo "No .zip or .rar files found under $DIR"
  exit 0
fi
...
echo ""
if [[ $errors -gt 0 ]]; then
  echo "Done with $errors error(s) — original RAR files kept."
  exit 1
fi
